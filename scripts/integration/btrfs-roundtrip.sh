...
COPY_MOUNT_OUTPUT="$(./target/release/vb-copy-mount open --provider btrfs --label copyview --target "${SNAPSHOT_MOUNT}" "${SOURCE_SUBVOL}")"
COPY_MOUNT_SNAPSHOT="$(awk '/^snapshot:/ {print $2}' <<<"${COPY_MOUNT_OUTPUT}")"
grep -q 'hello-from-btrfs' "${SNAPSHOT_MOUNT}/hello.txt"
./target/release/vb-copy-mount close --provider btrfs "${COPY_MOUNT_SNAPSHOT}" "${SNAPSHOT_MOUNT}"
